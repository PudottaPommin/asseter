...
func main() {
	if len(os.Args) < 2 {
		...
		fmt.Println("  gen   Generate bindata assets")
		os.Exit(1)
	}
...
func newAssetsHandlerFromCLI() (cmd *asseter.AssetsFsHandler, err error) {
	...
	flagSet := flag.NewFlagSet("gen", flag.ExitOnError)
	flagSet.StringVar(&o.Cwd, "cwd", "", "Working directory ( where are node_modules )")
	flagSet.StringVar(&o.Src, "src", "assets", "Directory for assets")
	flagSet.StringVar(&o.Out, "out", "bindata.asseter.go", "Output filename")
	flagSet.StringVar(&o.Pkg, "pkg", "assets", "Package name for generated file")
	if err = flagSet.Parse(os.Args[2:]); err != nil {
		return nil, err
	}

	cmd, err = asseter.NewAssetsFsHandler(o)
	if err != nil {
		return nil, err
	}
	return
